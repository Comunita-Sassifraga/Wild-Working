...
**`ORA_PROMEMORIA` è un'intenzione, non un orologio al minuto.** Il giro parte una sola volta al giorno, alle 16:00 in orario universale, cioè le 18:00 italiane d'estate e le 17:00 d'inverno. Per un promemoria serale quell'ora di scarto non cambia nulla, e costa una esecuzione al giorno invece di ventiquattro. Lo stesso vale per `ORA_PULIZIE`, programmato allo stesso modo alle 01:00 universali: le pulizie di §7 ragionano in giorni e in mesi interi, calcolati in `Europe/Rome`, e il minuto in cui partono non cambia il loro esito.

**La programmazione vive fuori dall'applicazione, e dove vive dipende dall'ospite.** Su Vercel sta in `vercel.json`, che chiama i due indirizzi dall'interno. Su Cloudflare non può starci: i Cron Triggers chiamano un Worker e non un indirizzo qualsiasi, quindi i due giri sono fatti partire da altrettante azioni programmate di GitHub (`.github/workflows/promemoria.yml` e `pulizie.yml`), che chiamano gli stessi due indirizzi dall'esterno, solo in `https`, presentando il `CRON_SECRET`. Le espressioni degli orari sono identiche nei due casi, perché entrambi leggono l'orario universale. In tutti e due i casi **chi cambia `ORA_PROMEMORIA` o `ORA_PULIZIE` deve cambiare anche la programmazione in uso: le due non si allineano da sole.**

**Un'azione programmata di GitHub si spegne da sola.** GitHub disattiva le azioni programmate di un repository pubblico dopo 60 giorni senza attività, e due mesi senza un commit sono del tutto normali per un'associazione di volontari. Se si fermano le pulizie si ferma anche l'anonimizzazione a 30 giorni (§5.3, §7), e si ferma in silenzio: nessuna esecuzione rossa, nessun avviso. Per questo una terza azione (`.github/workflows/segno-di-vita.yml`) scrive una data nel repository una volta al mese, così i sessanta giorni non scadono mai. Riguarda solo la strada GitHub: su Vercel il problema non esiste.
...
Un servizio separato avrebbe significato un secondo progetto Supabase e un secondo Vercel a pagamento, due informative, due serie di accordi con i fornitori, due superfici su cui può accadere un errore di privacy — e un residente costretto a registrarsi due volte, nella stessa settimana, per due servizi della stessa associazione.
