...
**La protezione anti-bot della zona ferma i giri notturni, e va tenuta spenta.** Il 15/09/2026, alla prima esecuzione vera dopo il rilascio, le due azioni sono fallite con un 403: Cloudflare rispondeva con una *managed challenge*, la pagina «Just a moment…», che un programma non può risolvere. La richiesta non arrivava nemmeno all'applicazione, quindi né il segreto né il codice del giro c'entravano. Le azioni di GitHub partono da indirizzi di centro dati, che Cloudflare sfida molto più volentieri di una connessione domestica: è il motivo per cui la stessa chiamata, provata da casa, rispondeva regolarmente. La causa era **Bot Fight Mode**, attivo sulla zona `sassifraga.org`; spegnendolo i due giri sono tornati a passare, e i due indirizzi continuano a rifiutare con 401 chi non presenta il `CRON_SECRET`.

Sul piano gratuito quella protezione **non ammette eccezioni per percorso**: non si può tenerla accesa e lasciar passare il solo `/api/mestieri/`. Chi dovesse riaccenderla fermerebbe i due giri, e con loro l'anonimizzazione a 30 giorni di §7. Almeno il guasto è rumoroso — GitHub manda un'email a ogni esecuzione rossa — al contrario dello spegnimento per inattività, dove il segnale è il silenzio. Se un giorno la protezione dovesse servire davvero, l'alternativa è far chiamare alle azioni l'indirizzo `workers.dev` del Worker, che non passa dalla zona: si cambia il solo segreto `URL_APP` **del repository**, non quello del Worker, che continua a scrivere il dominio pubblico dentro le email.

Spegnere Bot Fight Mode non è solo un rimedio a questo: la stessa protezione inietta uno script di Cloudflare in ogni pagina servita, e questo progetto tiene il carattere ospitato da sé e l'app libera da terze parti proprio per non far girare codice altrui nel browser dei visitatori (§14.5, rule 7). L'iniezione ha però un secondo interruttore, *JavaScript Detections*, che sopravvive al primo.
